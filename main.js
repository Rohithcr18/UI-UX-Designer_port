...
let projects = [
  {
    image: 'textures/ice.jpg',
    url: 'https://shorturl.at/mBq9d',
  },
  {
    image: 'textures/aque.jpg',
    url: 'https://shorturl.at/N1K89',
  },
  {
    image: 'textures/fruity.jpg',
    url: 'https://tinyurl.com/2a7h65m3',
  },
  {
    image: 'textures/foodd.jpg',
    url: 'https://tinyurl.com/yyjdrwrk',
  },
  {
    image: 'textures/game1.jpg',
    url: 'https://shorturl.at/4msi6',
  },
  {
    image: 'textures/game2.jpg',
    url: 'https://shorturl.at/naIfL',
  },
];
let aboutCameraPos = {
  x: 0.12,
...
let projectsCameraPos = {
  x: 0,      // Center the camera horizontally
  y: 0.5,    // Slight vertical offset
  z: 1.8,    // Move camera back for a wider view
};
let projectsCameraRot = {
  x: 0,      // No x-rotation for straight-on view
  y: 0,      // No y-rotation for straight-on view
  z: 0,
};
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('ROHITH C R', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      '          UI/UX Designe & Graphic Designer            ',
      {
        font: font,
        size: 0.018,
        height: 0,
...
function projectsMenuListener() {
  ...
  projects.forEach((project, i) => {
    ...
    const texture = textureLoader.load(
      ...
      function (error) {
        ...
        console.log('Full path attempted:', new URL(project.image, window.location.origin).href);
      }
    );
    ...
    // Position projects in a grid with better spacing
    const spacingX = 0.9;  // Horizontal spacing between projects
    const spacingY = 0.6;  // Vertical spacing between projects
    const startX = 0.5;   // Start further to the left
    const startY = 1.2;    // Vertical start position

    projectPlane.position.set(
      startX + (i % 3) * spacingX,  // 3 columns
      startY - Math.floor(i / 3) * spacingY,  // 2 rows
      -1.15
    );

    projectPlane.scale.set(0, 0, 0);
    // Store reference and position for animations
    projects[i].mesh = projectPlane;
    projects[i].y = startY - Math.floor(i / 3) * spacingY;
  ...
  projectsMenu.addEventListener('click', function (e) {
    ...
    gsap.to(camera.rotation, {
      ...
      duration: 1.5,
      onComplete: () => {
        // Show the 'Click Me' text after the camera animation completes
...
// skills menu
const skillsDropdown = document.querySelector('.skills-menu__dropdown');
document.getElementById('skills-btn').addEventListener('click', (e) => {
  e.preventDefault();
  skillsDropdown.classList.toggle('skills-menu__dropdown--open');
});

document.addEventListener('mouseup', (e) => {
  const contactContainer = document.querySelector('.contact-menu');
  const projectsMenu = document.getElementById('projects-menu');
  const clickMeText = document.getElementById('click-me-text');
  ...
  const skillsContainer = document.querySelector('.skills-menu');
  if (!skillsContainer.contains(e.target)) {
    skillsContainer
      .querySelector('.skills-menu__dropdown')
      .classList.remove('skills-menu__dropdown--open');
  }

  // Hide 'Click Me' text when clicking outside projects
  if (!projectsMenu.contains(e.target) && !document.querySelector('.project').contains(e.target)) {
    clickMeText.classList.remove('visible');
  }
});
